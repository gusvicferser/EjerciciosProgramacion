...
public class A2MostrarInformacionFicheros {
	...
	public static void main(String[] args) {
		...
		do {
			...
			if (!raiz.equals("")) { // Si la ruta es igual a nada (enter a secas) entonces salimos del bucle
				...
				try {
					muestraInfoRuta(busqueda); // Creamos una función como nos pide el ejercicio y la llamamos tratando
												// la excepción que provoca con un try catch
				} catch (Exception exc) {
					System.out.println(exc.getMessage());
				}
			} else {
				System.out.println(
						"Como decía Matias Prats: 'Hay una escasez de anestesia en los hospitales... Se acabó lo que sedaba'");
			}
			...
			if (archivosDentro.length != 0) {
				...
				for (File directorio : archivosDentro) { // Bucle for para mostrar los directorios.
					if (directorio.isDirectory()) {
						...
						System.out.println("[*] " + directorio.getName() + " " + fechaDir);
					}
				}
				...
				for (File archivo : archivosDentro) {
					if (archivo.isFile()) {
						...
						System.out.println("[A] " + archivo.getName() + " " + archivo.length() + " Bytes " + fechaArc);
					}
				}
			}
		}
